...
def predict_breed(image_id):
    MODEL_LOCATION = current_app.config["MODEL_PATH"] / "best_model.h5"
    CLASS_NAME_JSON = current_app.config["MODEL_PATH"] / "class_names.json"
    IMG_PATH = current_app.config["UPLOAD_PATH"] / str(image_id + ".jpeg")
    ...
    if not CLASS_NAME_JSON.is_file():
        print(
            f"ERROR: class_names.json not found at {CLASS_NAME_JSON}", file=sys.stderr
        )
        sys.exit(1)
    ...
    return {"breed": str(pred_breed), "confidence": pred_prob * 100}
